...
Image Deduper は、選択したフォルダーを走査し、SHA-1・pHash・SSIM で画像の重複を検出して画質の劣る方を `<source>/duplicates` に移動する Windows 11 向け WinUI 3 アプリです。GitHub で公開されている CLI ワークフローを GUI に発展させたものです。

CLI reference implementation / CLI 版: https://github.com/jogu6/imagededuper

## Download / ダウンロード

1. Open the repository’s **Releases** page on GitHub and select the latest version (for example `v1.0.0`).  
   GitHub の **Releases** ページを開き、最新バージョン（例: `v1.0.0`）を選択します。
